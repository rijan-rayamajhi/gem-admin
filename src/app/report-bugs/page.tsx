...
interface Release {
  ...
  publishDate: string;
  dueDate: string;
  appUrl?: string; // Added appUrl to the interface
}
    ...
    const fetchReleases = async () => {
      ...
      try {
        ...
        const data: Release[] = snapshot.docs.map(doc => ({
          ...
          appUrl: doc.data().appUrl, // Assuming appUrl is stored in the document
        }));
        setReleases(data);
      } catch (error) {
      ...
      <HomeLayout>
        <div className="p-6">
          ...
          <div className="mb-6">
            ...
            {/* APK Download Link */}
            {releases.length > 0 && releases[0].appUrl && (
              <div className="mb-2">
                <a
                  href={releases[0].appUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors shadow"
                >
                  Download Latest APK
                </a>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">APK</th>
                      ...
                      <tr key={release.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => window.location.href = `/report-bugs/${release.id}` }>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {release.appUrl ? (
                            ...
                            <span className="text-gray-400">No APK</span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <button
                            className="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors"
                            onClick={e => { e.stopPropagation(); window.location.href = `/report-bugs/${release.id}`; }}
                          >
                            View
                          </button>
                        </td>
